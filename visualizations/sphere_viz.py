...
class SpherePlannerViz:
    def __init__(
            self,
            planner: Union[SPARROWS_3D_planner,CROWS_3D_planner],
            plot_full_set: bool = False,
            t_plan: float = 0.5,
            t_full: float = 1.0,
            add_buffer: bool = True, # This is only for crows
            ) -> None:
        ...
        Args:
            ...
            add_buffer: Whether to add the unceratinty buffer for CROWS in the visualization
        '''
    ...
    def set_ka(self, ka: Union[Tensor, NDArray] = None):
        ...
        if ka is not None:
            ...
            if isinstance(self.planner, SPARROWS_3D_planner):
            ...
            elif isinstance(self.planner, CROWS_3D_planner):   
                with torch.no_grad():
                    buffer = self.planner.CROWS_constraint.confidence_bound
                    buffer = torch.hstack((torch.zeros(1,dtype=buffer.dtype,device=buffer.device),buffer))
                    qpos_nn = self.planner.CROWS_constraint.qpos_nn
                    qvel_nn = self.planner.CROWS_constraint.qvel_nn
                    time_nn = self.planner.CROWS_constraint.time_nn
                    ka_nn = self.ka.repeat(qpos_nn.size(0),1)
                    self.joint_centers = self.planner.CROWS_constraint.model.predict_centers(qpos_nn, qvel_nn, ka_nn, time_nn).transpose(0,1) 
                    radii = self.planner.CROWS_constraint.model.predict_radii(qpos_nn, qvel_nn, time_nn) + buffer * self.add_buffer
                    self.joint_radii = radii.transpose(0,1)
        else:
            self.failsafe = True
